  ...
  foreach(j=basinList, #basinList, ## Loop over basins
          .packages = c("data.table", "Rcpp", "RcppAlgos",
            ...
            if(length(DAMIDvec) <= minlazy) {
              ...
              permutlazy <- DamX[ESTAGIO_1 == "Planned", do.call(lazyExpandGrid, rep(list(c(0.1, 1)), .N))] 
              ...
              #Add all existing dams
              permut[, as.character(DamX[ESTAGIO_1 == "Operation", as.character(DAMID)]) := 0.1] #will return warning if currently no dams 
            }
